...
The default engine pipeline is JSON → uncompressed → Base64. Descriptors can override individual stages, for example `j;br;base64`, `j;gz;base64` or `application/json;brotli;base64`. JSON accepts `j`, `json`, and `application/json`; Brotli accepts `b`, `br`, and `brotli`; gzip accepts `g`, `gz`, and `gzip`; both compression formats are decoded with the environment’s native `DecompressionStream`; Base64 accepts both Base64 and Base64URL input through `from-base64`.
